...
Para a otimização do código, utilizamos a rotina apresentada em sala de aula: selecao -> cruzamento -> mutacao -> hall da fama.
- Para realizar a seleção dos indivíduos, utilizamos a seleção por torneio, que, no nosso caso, escolhe 3 indivíduos da população, calcula o `fitness` de cada um e, como queremos minimizar, o indivíduo que vence o torneio possui a menor energia total entre os três.

- Para o cruzamento, implementamos o `cruzamento de ponto simples`, porém com uma nova estratégia para garantir que, em qualquer cruzamento que ocorra, a diagonal da matriz será sempre 0 e o filho será uma matriz triangular superior. Isso é uma imposição realizada, pois somente há significado físico para cada indivíduo se a matriz obedecer a essas regras.
